...
|originalTransferMethods|originalTransferMethods|Transfer method used to initiate a session throughout all subsequent requests|
...
|signInTokenProtectionStatus|tokenProectionStatus|Token protection creates a cryptographically secure tie between the token and the device it's issued to. This field indicates whether the signin token was bound to the device or not|
